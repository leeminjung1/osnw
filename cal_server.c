...
int main(int argc, char **argv)
{
        ...
        for(;;)
        {
            ...
            printf("%08x %08x %08x ", ntohl(rdata.left_num), rdata.op, ntohl(rdata.right_num));
            ...
            printf("sending data:\n");
            printf("%08x %08x %08x %08x %08x", ntohl(rdata.left_num), rdata.op, ntohl(rdata.right_num), ntohl(rdata.result), ntohs(rdata.error));
            printf(" (%d %c %d %d %d)\n", ntohl(rdata.left_num), rdata.op, ntohl(rdata.right_num), ntohl(rdata.result), ntohs(rdata.error));
